...
Back in the code-server terminal, clone the project. Enter your GitLab username
(`root`) and password (`C1sco12345`) when Git prompts for credentials:
...
`main` branch exists on the server. Run these commands **only if** the clone was
empty or Git reported a warning:
...
into your `myproject` folder using the code-server Explorer — select the files,
**Copy**, then **Paste** into your project. The project must end up with this
structure:
...
├── configs/loopbacks.yaml         # declarative Loopback300
└── ansible/vars/vlans.yml         # VLAN intent — you edit this later
```

> **Tip:** If you prefer the terminal, you can create folders and files by hand
> (`mkdir <folder>` and edit with the code-server editor or `vi <file>`), but
> copy-and-paste from the solution folder is faster and less error-prone.
...
```bash
git add .
git commit -m "add pipeline project files"
git push
```
...
the remote from the **Source Control** panel — the same edit-test-push loop, now
from the IDE.
