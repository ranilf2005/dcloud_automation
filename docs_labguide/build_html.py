#!/usr/bin/env python3
"""
build_html.py — render LAB_GUIDE.md into a single self-contained index.html.

The Markdown is embedded inside the HTML (no fetch), so the page renders when
opened directly from disk (file://) or served by GitLab Pages — no server or
CORS workaround required. Diagrams use Mermaid; code uses highlight.js (via CDN).
...
md = (HERE / "LAB_GUIDE.md").read_text(encoding="utf-8")
...
<title>dCloud Automation — Training Lab Guide</title>
<meta name="description" content="Step-by-step training lab guide: NetDevOps CI/CD (pyATS, Ansible, GitLab) and Cisco Security IaC (FMC/FTD with Terraform, REST API, Ansible).">
...
#toc a{color:var(--muted);padding:5px 10px;border-radius:7px;font-size:.9rem;border-left:2px solid transparent}
#toc a:hover{background:var(--panel2);color:var(--text);text-decoration:none}
#toc a.lvl3{padding-left:22px;font-size:.83rem}
...
<aside class="sidebar" id="sidebar">
  <div class="brand">dCloud<span class="dot">.</span>Automation<small>Training Lab Guide</small></div>
...
(function(){
  ...
  // Build the sidebar TOC from h2 + h3.
...
out = TEMPLATE.replace("__MARKDOWN__", md_safe)
(HERE / "index.html").write_text(out, encoding="utf-8")
print("Wrote {} ({:,} bytes)".format(HERE / "index.html", len(out)))
